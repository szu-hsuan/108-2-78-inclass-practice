...
####餘數：`%%`,用來判斷某種規律的結果eg.奇偶數
####次方：`**`(python) 或 `^`(javascript) [R兩個都可以]
...
當運算的兩物件內容長度不同時，則將其中較短的一個反複堆疊到可以長得跟另一個一樣高時才來運算，Recycling。
```{r}
# 狀況一: 堆疊一定倍數剛好一樣長
c(2,3)/c(-2,-13,10,22)
...
example裡誰的身高大於等於170
```{r}
example$name[c(T, F, T, F)] # but no efficiency
```
##
```{r}
...
```{r}
example$birthday
class(example$birthday) #比較的class 要一樣（ Date >= Date)
typeof(example$birthday)#double = 數值
```
```{r}
# False (不會顯示error,但不是你要的結果)
# because Date >= Character
...
＊＊order(f(x)) 
=> f(x) 的資料已經由小到大排序，所以須確定level(y)的排列順序
```{r}
example$allowance <- 
  ordered(example$allowance)
```

再檢查example$allowance是否改變,再執行149~154
```{r}
...
```{r}
levels(list_victimAges_female$年齡層) <- levels_new #回存
levels_new
```
...
*NA是特殊的，可直接在各class，不用顧慮要不要加`""`（character)
```{r}
x2 <- c(1,NA,10)
y2 <- c(-1,NA,20)
...
# in logical type, True=1,False=0
...
*`typeof(f(c))`: R 本質是如何看待f(c)
*`class`:把物件根據能夠他進行的運算分類
```{r}
...
####`factor` 是用數字在存
####`stringr::str_detect` ：用print 在screen上的樣子在偵測
```{r}
...
**Examples**：如果有看沒有懂，複製，執行。
```{r}
fruit <- c("apple", "banana", "pear", "pinapple")
str_detect(fruit, "a")
str_detect(fruit, "^a") # "^a" : regular expression
str_detect(fruit, "a$")
str_detect(fruit, "b")
...
```
*expression sheet means `data.frame`
找出問題中包含“肺炎”字眼的問題。
##??
```{r}
