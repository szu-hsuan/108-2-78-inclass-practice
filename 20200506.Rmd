...
str_sub(c("410873002","410552103"),5, 6)
```
...
**lubridate對年只有西元年的概念。**
要能想辦法把上面的“108”,“98”取出加上1911；再塞回去
...
####字「群」組合(..|..|..): strip上的出象會出現一組字時用，如“AB|ab”/ `(AB|ab)`, 可以產生“AB”或“ab”兩種出象。
...
str_view_all(studentIds, "107|108") # 前後沒東西的情況，可不寫()
...
str_view_all(headline, "[謝陳官]") # 有1個strip: 出象有3個可能字元 (謝、陳、官)
str_count(headline, "[謝陳官]")

str_view_all(headline, "謝無雙|官求敗") # 有1個strip，出象有2個字組；沒寫()，因為前後沒東西
str_count(headline, "謝無雙|官求敗")

str_view_all(headline, "陳[^官]") 
# 固定字元＋有1個strip： 出象為排官的字元；strip[字元strip]
...
strip4 <- "號"
#也可以是([0-9]{3})
myRegexSlotMachine <- paste0(strip1,strip2,strip3,strip4) # XXX號
#也可以是([0-9]{3})
print(myRegexSlotMachine)
...
print(myRegexSlotMachine)
str_view_all(addresses,myRegexSlotMachine) #看一下
str_extract(addresses,myRegexSlotMachine) #萃取出來

str_view_all(addresses,"[0-9]+") # 樓號也會選到
```
*but, 如果只在乎數字，要先把“號”選出來，再刪掉
...
```{r}
# 目標是前面的唧唧
target_regex <- "唧唧"  #固定字組
pattern <- glue::glue("^{target_regex}")
# 在environment 中看有沒有`{}`的物件，撈出來，塞回去
print(pattern)
...
**`a(?=c)` : a followed by c
target後面是B：target_regex(?=B_regex)： target_pattern後面是年 [a,c皆是_regex ]
