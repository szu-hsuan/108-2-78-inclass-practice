...
**decreasing=T：If miss this, the ranking will from small to big**
...
**若要針對向量或list裡的每個元素進行相同操作，purrr是相當方便的工具，最常用的是：**
```{r}
...
3.選出每個活動showInfo下的每場次之「單場次演出時間」及「場地名稱」。（只用到map, 及~。）此題有點難，考驗同學map的活用能力。 答案應符合的str:
前兩個元素應有的str:   
List of 34  
 $ :List of 1  # 第一個元素  
  ..$ :List of 2  
...
jsonlite::fromJSON("https://data.ntpc.gov.tw/od/data/api/67BB3C2B-E7D1-43A7-B872-61B2F082E11B?$format=json", simplifyDataFrame = F) ->
  busRoutes
```
找出Id(路線代碼)、providerId(業者代碼)、providerName(業者中文名稱)、nameZh(中文名稱)。

##as.list/as.data.frame

*data.frame是list底下所有元素若為向量，其length均相同的一個特殊結構。
*as.list(), as.data.frame()

```{r}
char <- c("A","A","C","B","C","D")
table(char) -> tb_char
as.list(tb_char) -> list_tb_char
as.data.frame(tb_char) -> dataFrame_tb_char
```

```{r}
vector1 <- c(T,F,F,T,T)
names(vector1) <- c('Mary','John','Bruce','Jack', 'Jane')
vector1

as.list(vector1) -> list_vector1
as.data.frame(vector1) -> dataFrame_vector1
```
...
####人力資源調查非勞動力之未參與勞動原因
```{r}
jsonlite::fromJSON("https://www.dropbox.com/s/ck7pc59tkf568xq/discouragedWorkers.json?dl=1") -> discouragedWorkers
```

項目別_Iterm 裡沒有M的資料選出來後可以形成年資料，當中“想工作而未找工作且隨時可以開始工作”的人稱之為「怯志工作者」。請問在年資料裡哪一年的怯志工作者佔總數比例最高。

####5 台北大學正門的座標是 121.373323, 24.945398。算出它與垃圾車經過所有點（由longitude及latitude）的直線經緯座標距離存在distance新物件。

利用min()函數求得distance裡元素值的最小值，及產生：distance為最小值的「要/不要」向量pick_minDistance。

如果你要在台北大學正門等垃圾車，最晚不得晚於何時？(只「要」distance為最小值下，自garbageRoutes的time取出元素值。)
