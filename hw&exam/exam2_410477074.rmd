...
*不可以這樣寫，會變成更改tpe_highSchools*
*what you should do is set levels.*
```{r ans11}
...
a <- "[:digit:]{15,16}"
a1 <- as.integer(a)
data228$年 <- str_extract(data228$檔號,a,)
# data228[c("全宗名編號","年")]
```
...
```{r ans22}
choose <- data228$個人名稱
levels(str_detect(data228$附註[[1]],data228$個人名稱)
# victims
```
...
```{r ans31}
levels(factor(courseStructure$應修系級))
pattern <- "(不動產城鄉系|財政系|電機系|資工系|法律學系財經法組|法律學系法學組|法律學系司法組|會計系|統計系|金融系|社學系|社工系|通訊系|電機系|企管系|休運系|經濟系|外語系|歷史系|行政系|中文系|通訊系)[:digit:]+"

str_extract(courseStructure$應修系級,pattern) ->majorGradeCombos
...
```{r ans32}
pick <- str_detect(as.character(courseStructure$應修系級),"經濟系3")
levels(factor(courseStructure$科目名稱[pick])) -> econ101junior
econ101junior
```
...
```{r ans33}
select <- str_detect(as.character(courseStructure$應修系級),courseStructure)
map(courseStructure$應修系級,)
