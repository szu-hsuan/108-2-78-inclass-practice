...
ymd_hms(taiwan_time)
dmy_hms(oslo_time)
with_tz(ymd_hms(taiwan_time),tzone="America/Chicago")
with_tz(dmy_hms(oslo_time),tzone="America/Chicago")

two_times_chicago <-c(with_tz(
  ymd_hms(taiwan_time),tzone="America/Chicago"),
  with_tz(dmy_hms(oslo_time),tzone="America/Chicago"))
two_times_chicago
```
...
```{r ans33}
a <- ymd("2014-01-01")
accidents <- list("time"=a,
                  "bureaus"=list(
                    "板橋分局"=list(                    
"type"="駕駛人過失-其他","casualties"=1),

"海山分局"=list(
"type"="駕駛人過失-搶越行人穿越道","casualties"=1
                    )
                  ))
str(accidents)
accidents
